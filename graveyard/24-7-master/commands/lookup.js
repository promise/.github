module.exports = {
  ...
  usage: {
    "<id>": "The ID you want to lookup."
  },
  examples: {},
  aliases: [ "bot-lookup", "id-lookup" ],
...
module.exports.run = async (client, message, args, permissionLevel, config, constants) => {
  try {
    ...
    if (!user.bot) return message.channel.send(":white_check_mark: This ID is definitely a user ID of " + user.username + "#" + user.discriminator + " (" + user.id + ").");
    if (user.bot) {
      let botMsg = await message.channel.send(":white_check_mark: This ID is definitely a bot ID of " + user.username + "#" + user.discriminator + " (" + user.id + "), fetching more information...")
        try {
          let botblock = await fetch("https://botblock.org/api/bots/" + user.id).then(res => res.json())
          if (botblock.discriminator == "0000") return botMsg.edit(":white_check_mark: This ID is definitely a bot ID of " + user.username + "#" + user.discriminator + " (" + user.id + "), unfortunately this bot is not listed on any BotBlock-supported bot lists.")
          ...
          return botMsg.edit(":white_check_mark: This ID is definitely a bot ID of " + user.username + "#" + user.discriminator + " (" + user.id + "):", {
              embed: {
      ...
      } catch(e) { console.log(e); return botMsg.edit(":white_check_mark: This ID is definitely a bot ID of " + user.username + "#" + user.discriminator + " (" + user.id + "), unfortunately we could not get a good connection to BotBlock."); }
    }
  } catch(e) {}
  ...
  try {
    ...
    if (legacy) {
      ...
      if (invite.guild.verification_level) add({ "Verification Level": ["None", "Verified email", "Verified email and 5 minutes on Discord", "Verified email and 10 minutes on server", "Verified phone"][invite.guild.verification_level] })
  ...
  if (!message.guild) return message.channel.send(":x: The ID is unknown for me. Soarry.");
  
  let channelIDs = message.guild.channels.map(ch => ch.id);
  if (channelIDs.includes(args[0])) return message.channel.send(":white_check_mark: This ID is definitely a channel ID of channel " + message.guild.channels.get(args[0]) + ".");
  
  let roleIDs = message.guild.roles.map(r => r.id);
  if (roleIDs.includes(args[0])) return message.channel.send(":white_check_mark: This ID is definitely a role ID of role " + message.guild.roles.get(args[0]).name.replace("@", "@.") + "." + (message.guild.roles.get(args[0]).name == "@everyone" ? "\n**Fun fact:** Did you know the server ID and the @.everyone-ID is the same?" : ""));
  
  try {
    let res = await fetch("https://cdn.discordapp.com/emojis/" + args[0] + ".png")
    if (res.ok) return message.channel.send(":white_check_mark: This ID is definitely a custom emoji ID. [<https://cdn.discordapp.com/emojis/" + args[0] + ".png>]")
  } catch(e) {}

  if (args[0].length == 15) return message.channel.send(":white_check_mark: This ID looks like an Auttaja punishment ID.");
  
  let msg = await message.channel.send(":warning: Checking if the ID is a message in this guild. This may take a while depending on rate limiting.")
  ...
  for (var c in channels) {
    try {
      ...
      known = true; return msg.edit(":white_check_mark: This ID is definitely a message ID. [<https://discordapp.com/channels/" + [message.guild.id, m.channel.id, m.id].join("/") + ">]");
    } catch(e) {}
  }
  
  return msg.edit(":x: The ID is unknown for me. Soarry.");
}
